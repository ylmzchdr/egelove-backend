        ...
        <div className="mx-auto max-w-2xl px-4">
          <div className="bg-pink-50 border border-pink-200 rounded-xl p-8">
            <div className="flex justify-between mb-2">
              ...
              <span className="text-[#8A6372] text-base">{step + 1}/{STEPS.length}</span>
            </div>

            <div className="w-full bg-pink-50 rounded-full h-2 mb-8">
            ...
            {step === 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><Label>Ad</Label><Input className="bg-pink-950/50 border-pink-200 text-[#2D1721]" value={form.name} onChange={(e) => update("name", e.target.value)} /></div>
                <div><Label>Soyad</Label><Input className="bg-pink-950/50 border-pink-200 text-[#2D1721]" value={form.surname} onChange={(e) => update("surname", e.target.value)} /></div>
                <div><Label>Doğum Tarihi</Label><Input type="date" className="bg-pink-950/50 border-pink-200 text-[#2D1721]" value={form.birthDate} onChange={(e) => update("birthDate", e.target.value)} /></div>
                <div>
                  <Label>Cinsiyet</Label>
                  <Select value={form.gender} onValueChange={(v) => update("gender", v)}>
                    <SelectTrigger className="bg-pink-950/50 border-pink-200 text-[#2D1721]"><SelectValue placeholder="Seçin" /></SelectTrigger>
                    <SelectContent>
                      {GENDER_OPTIONS.map((o) => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div><Label>Telefon (isteğe bağlı)</Label><Input className="bg-pink-950/50 border-pink-200 text-[#2D1721]" value={form.phone} onChange={(e) => update("phone", e.target.value)} /></div>
                <div>
                  <Label>İl</Label>
                ...
                <div>
                  ...
                  <Select value={form.districtId ? String(form.districtId) : ""} onValueChange={(v) => update("districtId", parseInt(v))}>
                    <SelectTrigger className="bg-pink-950/50 border-pink-200 text-[#2D1721]"><SelectValue placeholder="Önce il seçin" /></SelectTrigger>
                    <SelectContent>
            ...
            {step === 1 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><Label>Boy (cm)</Label><Input type="number" className="bg-pink-950/50 border-pink-200 text-[#2D1721]" value={form.height || ""} onChange={(e) => update("height", e.target.value ? parseInt(e.target.value) : 0)} /></div>
                <div><Label>Kilo (kg)</Label><Input type="number" className="bg-pink-950/50 border-pink-200 text-[#2D1721]" value={form.weight || ""} onChange={(e) => update("weight", e.target.value ? parseInt(e.target.value) : 0)} /></div>
            ...
            {step === 2 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><Label>Meslek</Label><Input className="bg-pink-950/50 border-pink-200 text-[#2D1721]" value={form.occupation} onChange={(e) => update("occupation", e.target.value)} /></div>
            ...
            {step === 3 && (
              <div className="space-y-4">
                ...
                <textarea className="w-full bg-pink-950/50 border border-pink-200 rounded-lg p-3 text-[#2D1721] min-h-[120px] resize-y" placeholder="Hobilerin, ilgi alanların, hayallerin..." value={form.aboutMe} onChange={(e) => update("aboutMe", e.target.value)} />
                <Label className="text-lg">Partnerinde aradığın özellikler</Label>
                <textarea className="w-full bg-pink-950/50 border border-pink-200 rounded-lg p-3 text-[#2D1721] min-h-[80px] resize-y" placeholder="Nasıl birini arıyorsun?" value={form.lookingFor} onChange={(e) => update("lookingFor", e.target.value)} />
                <Label>Hobiler (virgülle ayırın)</Label>
                <Input className="bg-pink-950/50 border-pink-200 text-[#2D1721]" placeholder="Yüzme, kitap okuma, seyahat" value={form.hobbies} onChange={(e) => update("hobbies", e.target.value)} />
              </div>
            )}
            ...
            {step === 4 && (
              <div>
                ...
                <ProfilePhotoUpload />
                <p className="text-[#8A6372] text-base mt-4">En az 1 fotoğraf yüklemen önerilir. Fotoğraflar onaylandıktan sonra profilin görünür olacak.</p>
              </div>
            )}

            <div className="flex gap-3 mt-8">
              {step > 0 && (
                <Button variant="outline" className="border-pink-300 text-[#2D1721] flex-1" onClick={() => setStep(step - 1)}>
                  Geri
                </Button>
              )}
              <Button
                className="bg-pink-600 hover:bg-pink-700 text-[#2D1721] flex-1 disabled:opacity-50"
                onClick={saveAndNext}
